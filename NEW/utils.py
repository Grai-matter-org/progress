...
def fit_and_eval_cls(
    ...
    pipe = Pipeline([("prep", preproc), ("lgbm", clf)])
...
def to_long_with_offsets(df, id_col="encounter_id"):
    base = df[
        [id_col, "encounter_start_dt", "proc_times_seq_str", "proc_descs_seq_str"]
    ].copy()

    base["encounter_start_dt"] = pd.to_datetime(
        base["encounter_start_dt"], errors="coerce"
    )
    ...
    base["proc_times_list"] = (
        base["proc_times_seq_str"].fillna("").astype(str).str.split("|")
    )
    base["proc_descs_list"] = (
        base["proc_descs_seq_str"].fillna("").astype(str).str.split("|")
    )
    ...
    long = long[
        (long["proc_times_list"] != "") & (long["proc_descs_list"] != "")
    ].copy()
...
def build_day_level_df(df, long_df, procedure, max_day=7, id_col="encounter_id"):
    ...
    proc_days = (
        long_df[long_df["proc_desc"] == procedure][[id_col, "days_after_start"]]
        .rename(columns={"days_after_start": "day"})
        .assign(occurs=1)
    )

    out = (
        days.merge(proc_days, on=[id_col, "day"], how="left")
        .fillna({"occurs": 0})
